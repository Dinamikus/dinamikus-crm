...
- "ubicacion_mencionada" solo aplica cuando el paso actual pregunta por la sede; en cualquier
  otro caso va null.
...
async function manejarServicio(interp, estado, tenantId, leadId, conversationId) {
  ...
  const texto = (interp.servicio_mencionado || '').toLowerCase();
  const match = servicios.rows.find((s) => texto && s.nombre.toLowerCase().includes(texto));
...
async function manejarUbicacion(interp, estado, tenantId, leadId, conversationId) {
  ...
  const texto = (interp.ubicacion_mencionada || '').toLowerCase();
  const match = ubicaciones.rows.find((u) => texto && u.nombre.toLowerCase().includes(texto));
