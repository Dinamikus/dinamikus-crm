...
- **Contactos `@lid` resueltos a su teléfono real**: WhatsApp identifica a algunos contactos con un identificador de privacidad (`@lid`) en vez de su número directo. Baileys trae el teléfono real en `senderPn` en la mayoría de los mensajes **entrantes**, pero casi nunca en los mensajes que el asesor envía **desde su propio celular** (`fromMe: true`) — así que un lead puede tener su teléfono resuelto, pero un mensaje saliente posterior solo trae el LID.
  - **Se guardan AMBOS identificadores en el mismo lead** (`phone` y `external_user_id` = LID) para que cualquiera de los dos encuentre la conversación correcta — antes, un mensaje saliente sin `senderPn` no encontraba el lead ya creado y ese mensaje se perdía del hilo sin avisar. Confirmado con datos reales de producción y corregido.
  - Si el *primer* mensaje de un contacto llega sin `senderPn` (crea el lead solo con el LID) y uno *posterior* sí lo trae, el lead existente se **completa** con el teléfono en vez de crear uno duplicado.
- **`sql/migrations/009_backfill_lid_phone_numbers.sql`** — si ya tenías leads de WhatsApp QR guardados *antes* de este arreglo (con teléfono vacío), esta migración recupera el número real desde el `senderPn` que ya estaba guardado en el mensaje original. Es segura de correr más de una vez.
...
- **Inbox en móvil (lista/detalle)**: el Inbox usa un layout de dos columnas fijas (lista de conversaciones + hilo), que en pantallas angostas dejaba el hilo empujado fuera de la vista — al abrir una conversación *sí* se renderizaba, solo que invisible sin scroll horizontal, así que parecía que el clic no hacía nada. En móvil ahora se muestra una sola columna a la vez: la lista, o el hilo con un botón "← Volver a conversaciones" para regresar. No afecta el layout de escritorio (sigue siendo lista + hilo lado a lado).
