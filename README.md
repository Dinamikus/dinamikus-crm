...
- **Contactos `@lid` resueltos a su teléfono real**: WhatsApp identifica a algunos contactos con un identificador de privacidad (`@lid`) en vez de su número directo. Baileys igual trae el teléfono real en el campo `senderPn` del mensaje — lo usamos para guardar siempre el número, no el LID, que es lo que le sirve al negocio para campañas de recontacto. Si por algún motivo `senderPn` no viene en el mensaje (poco común), el lead se guarda con el LID como identificador de respaldo, para no perder la conversación.
